...
class Transaction extends Model implements Segregatable, Recyclable, Clearable, Assignable
{
    ...
    public static function transactionNo(string $type, Carbon $transaction_date = null)
    {
        ...
        return $type . str_pad((string) $period_count, 2, "0", STR_PAD_LEFT)
            . "/" .
            str_pad((string) $next_id, 5, "0", STR_PAD_LEFT);
    }
    ...
    public function addLineItem(LineItem $lineItem): void
    {
        ...
        if ($lineItem->account_id == $this->account_id) {
            throw new RedundantTransaction($this->account_id.' - '.$lineItem->account_id);
        }
    ...
    public function client(){
        return $this->belongsTo('App\Models\Client\Client', 'id', 'customer_id');
    }
